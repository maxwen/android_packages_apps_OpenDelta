...
public class DeltaInfo {
	...
	public class FileBase {
		...
		public FileSizeMD5 match(File f, boolean checkMD5, ProgressListener progressListener) { return null; }
	}
	...
	public class FileUpdate extends FileBase {
		...
		public FileSizeMD5 getUpdate() { return update; }
		public FileSizeMD5 getApplied() { return applied; }

		public FileSizeMD5 match(File f, boolean checkMD5, ProgressListener progressListener) {
			if (f.exists()) {
				if (f.length() == getUpdate().getSize()) if (!checkMD5 || getUpdate().getMD5().equals(getFileMD5(f, progressListener))) return getUpdate();
				if (f.length() == getApplied().getSize()) if (!checkMD5 || getApplied().getMD5().equals(getFileMD5(f, progressListener))) return getApplied();
			}
			return null; 
		}
	}
	...
	public class FileFull extends FileBase {
		...
		public FileSizeMD5 getOfficial() { return official; }
		public FileSizeMD5 getStore() { return store; }
		public FileSizeMD5 getStoreSigned() { return storeSigned; }

		public FileSizeMD5 match(File f, boolean checkMD5, ProgressListener progressListener) {
			if (f.exists()) {
				if (f.length() == getOfficial().getSize()) if (!checkMD5 || getOfficial().getMD5().equals(getFileMD5(f, progressListener))) return getOfficial();
				if (f.length() == getStore().getSize()) if (!checkMD5 || getStore().getMD5().equals(getFileMD5(f, progressListener))) return getStore();
				if (f.length() == getStoreSigned().getSize()) if (!checkMD5 || getStoreSigned().getMD5().equals(getFileMD5(f, progressListener))) return getStoreSigned();
			}
			return null; 
		}
	}
	...
	public DeltaInfo(byte[] raw, boolean revoked)  throws JSONException, UnsupportedEncodingException, NullPointerException {
	...
	public FileFull getIn() { return in; }
	public FileUpdate getUpdate() { return update; }
	public FileUpdate getSignature() { return signature; }
	public FileFull getOut() { return out; }
	...
	private String getFileMD5(File file, ProgressListener progressListener) {
		...
		if (progressListener != null) progressListener.onProgress(getProgress(current, total), current, total);
